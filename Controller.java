...
/*
* The Controller class is responsible for starting overall execution (main method)
* and intializing the GameBoard and GameWindow classes. It also listens for input 
* and controls the overall game logic and state control. 
* For the most part it shouldn't be edited however there are some top level settings 
* such as the isTurnBased or hasFog variables you can change. 
*/
public class Controller{
	...
	private ArrayList<Creature> creatures; // creatures currently on the board
...
// Edit this if you like to make your game realtime!
	...
	private boolean hasFog = true; // Part 4
	...
	public void loadBoardFromFile(int boardID){
		...
		initializeBoard(); // sharing an initialize method since we have more code to set up
	}
	...
	public void loadBoard(int boardID){
		...
		// sets up local references for player, creature and the fog
		initializeBoard();
    }
    ...
    public void initializeBoard(){
    	...
    	// Are we using the fog setting? 
		if( hasFog){
			board.hideTiles();
			if( currentPlayer != null)
				currentPlayer.setVision();
		}
    }
	...
	private void resolveConflicts(){
		...
		if(currentTile.hasCreature()){
			...
			if(battleCreature(tempCreature)){
				...
				creatures.remove(tempCreature); // Remove from the Creature list
			}else{
				// Lost Battle
	...
	public void checkEndConditions(){
		...
		if(currentPlayer.getHitPoints() <= 0 ){
			gameOver(false);
		// Player has won
		}else if(creatures != null && creatures.size() == 0 && currentPlayer.getItemCount() > 0){
			System.out.println("Game Over " + creatures.size());
			gameOver(true);
		}
	}
