...
/*
* GameBoard contains the main state of the game. This holds all the Tiles, 
* which in turn hold the Player, Creature and Items.
*/
public class GameBoard{
	...
	// Added for A4 - Access a Tile directly
	// Q2
	...
	// Q4 - Fog 
	// Hide all tiles that are not visible
	public void hideTiles(){
		// set all tiles to have visibility off
	...
	public void setBoard(char[][] newBoard){
		...
		for(int row = 0; row < newBoard.length; row++){
			for(int col = 0 ; col< newBoard[row].length; col++){
				...
				if( DEBUG_MODE){
					// If debug mode, assign row /col description to the tiles. 
					map[row][col].setDescription("Tile(" + row+ ","+col+")");
				}
			}
		}
	...
	public Tile createTile(char code){
		...
		if(code == 'X'){
			...
			// Create a trap tile and return it
			returnTile = new TrapTile();
			
		}else if(code == 'H'){
			// Create a healing tile and return it
			returnTile = new HealingTile();
		}
	...
	public char getTileCode(Tile currentTile){
		...
		// Check if this tile is a TrapTile
		}else if( currentTile instanceof TrapTile){
			returnChar = 'T';
		// Check if this tile is a HealingTile
		}else if( currentTile instanceof HealingTile){
			returnChar = 'H';
		}
		return returnChar;
	}
	...
	private void setOneTileNeighbors(int row, int col){
		...
		neighbors[0] = map[(row - 1 + getHeight()) % getHeight()][col];// North
		neighbors[1] = map[row][(col + 1 + getWidth()) % getWidth()];// East
		neighbors[2] = map[(row + 1 + getHeight()) % getHeight()][col];// South
		neighbors[3] = map[row][(col - 1 + getWidth()) % getWidth()];// West
		...
		for(int row = 0; row < map.length; row++){
			for(int col = 0; col < map[row].length; col++){
				...
				if(map[row][col].hasCreature()){
					tempList.add(map[row][col].getCreature());
				}
			}
		}
